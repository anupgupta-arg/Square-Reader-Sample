...
#import <SquareReader/PPINContext.h>
...
/**
 ...
 @param bypassAllowed BOOL value indicating whether bypassing PIN entry is allowed.
 @param lastAttempt BOOL value indicating if the received PIN request is the final attempt allowed on the card.
 @param digitEntryView The view containing the screen for entering a PIN.
 */
- (void)omnichannelCommsFeature:(CROmnichannelCommsFeature *)feature didReceiveInputRequestWithPaymentCard:(CRPaymentCard *)paymentCard bypassAllowed:(BOOL)bypassAllowed lastAttempt:(BOOL)lastAttempt digitEntryView:(DEDigitEntryView *)digitEntryView;
...
/// Submit the encrypted PIN data blob to the reader for further processing.
- (BOOL)submitInput:(NSData *)pinblock;

/// Configure the secure PIN Pad handler.
- (BOOL)configureInputWithContext:(id<PPINContext>)context;

/// Start the secure touch entry module.
- (BOOL)startInputWithRect:(CGRect)rect orientation:(UIInterfaceOrientation)orientation;

/// Update the active secure touch entry module's coordinate system.
- (BOOL)updateInputWithRect:(CGRect)rect orientation:(UIInterfaceOrientation)orientation;

/// Stop the active secure touch entry module.
- (void)stopInput;
